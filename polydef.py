...
def chebroots(n):
    #Return roots of nth order chebychev polynomial.
    roots = np.zeros(n)
    for i in np.arange(n):
        roots[i] = np.cos( (np.pi*(np.float(i+1)-0.5))/np.float(n) )
    return(roots)
...
def chebpoly(x,n):
    ...
    cheb = np.zeros(n)
    if (n < 1):
        sys.exit('fatal chebpoly error n must be greater than 1 (polydef)')
    elif (n == 1):
        cheb[0] = 1.0
    else:
        cheb[0] = 1.0
        ...
        for j in np.arange(n-2):
            cheb[j+2] = 2.0*x*cheb[j+1]-cheb[j]
    return(cheb)

def chebpolytensor(xx,nmsv,npoly,ncheb):
    #Returns vectof chebychev polynomials.

    cheb_i = np.zeros(npoly)
    chebtensorm1 = np.zeros(npoly)
    chebtensor = np.zeros(npoly)
    ncheballm1 = 1
    chebtensor[0] = 1.0
    for i in np.arange(nmsv-1,-1,-1):
        cheb_i[:ncheb[i]] = chebpoly(xx[i],ncheb[i])
        chebtensorm1[:ncheballm1] = chebtensor[:ncheballm1]
        for k in np.arange(ncheb[i]):
            k1 = k*ncheballm1
            chebtensor[k1:k1+ncheballm1] = cheb_i[k]*chebtensorm1[:ncheballm1]
        ncheballm1 = ncheb[i]*ncheballm1
    return(chebtensor)
 
def nonsparsegrid(nmsv,npoly,ncheb):
    #Return chebychev grid and associated matrices.
    #form smolyak grid
    chebpoints = np.zeros(npoly)
    xgrid = np.zeros([npoly,nmsv])
    for ie in np.arange(nmsv-1,-1,-1):
        if (ie == nmsv-1):
            blocksize = 1
        else:
            blocksize = blocksize*ncheb[ie+1]
        ncall = np.int(npoly/(blocksize*ncheb[ie]))
        chebpoints[:ncheb[ie]] = chebroots(ncheb[ie])
        for ic in np.arange(ncall):
            for ib in np.arange(ncheb[ie]):
                xgrid[ncheb[ie]*blocksize*ic+blocksize*ib:ncheb[ie]*blocksize*ic+blocksize*(ib+1),ie] = chebpoints[ib] 

    bbt = np.zeros([npoly,npoly])
    for i in np.arange(npoly):
        bbt[i,:] = chebpolytensor(xgrid[i,:],nmsv,npoly,ncheb)
...
def get_linspline(xx,ss,ind_ss,acoeff,exoggrid,steps,nfunc,ngrid,npoly,nmsv,ncheb,ne):
    #returns decision rule
    linspline = np.zeros([nfunc])
    y1 = np.zeros([nfunc])
    y2 = np.zeros([nfunc])
    
    poly_xx = chebpolytensor(xx,nmsv,npoly,ncheb)
